...
const sendSlackMessage = async function(results, test, testMetaData, testVariables){

    try {
        var slack_token = await secretmanager.getSecretValue('sanity_runner/slack_api_token')
    } catch(err) {
        console.log(err)
    }
    ...
    if(testVariables.hasOwnProperty('ALERT')){
        ...
        if(testVariables.ALERT && testVariables.numFailedTests > 0){
            for (test of Object.keys(testFiles)){
                ...
                await sendSlackMessage(results, test, testMetaData, testVariables)
            }
        }
    }
}