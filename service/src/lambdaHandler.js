...
if (process.pid === 1) {
    ...
    process.on('SIGCHLD', function() {
        let waitPidResult
        while ((waitPidResult = waitpid2.waitpid(-1, 0 | waitpid2.WNOHANG))['return'] > 0) {
            if (waitPidResult.exitCode != null) {
                console.log("A child process exited normally by calling exit", waitPidResult)
            } else if (waitPidResult.signalCode != null) {
                console.log(`child process ${waitPidResult['return']} terminated because it was sent signal ${waitPidResult.signalCode}`)
            } else {
...
module.exports.handler = async function(event, context, callback) {
    ...
    const chrome = new ChromeInstaller({
        ...
        s3Key: 'HeadlessChrome-66.0.3343.0.tar.gz',
        debug: process.env.DEBUG || false,
    })
    await chrome.setupChrome()
    const runner = new TestRunner(chrome.executablePath)
    const testResults = await runner.runTests(event.testFiles, event.testVariables)
